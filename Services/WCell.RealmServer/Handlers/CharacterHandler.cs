...
namespace WCell.RealmServer.Handlers
{
    ...
    public static class CharacterHandler
    {
       ...
       /* public static void CharEnumRequest(IRealmClient client, RealmPacketIn packet)
                    ...
                    foreach (var record in chrs)
                    {
                        ...
                        if (record.JustCreated)
                        {
                            ...
                            if (archetype != null)
                            {
                                ...
                                foreach (var item in items)
                                {
                                    ...
                                    {
                                        packet.Position = itemOffset +
                                                          ((int) template.EquipmentSlots[0]*CharEnumItemBytes);
                        ...
                        else
                        {
                            foreach (var item in record.GetOrLoadItems())
                            {
                                ...
                                var template = item.Template;
                                if (item.ContainerSlot == BaseInventory.INVALID_SLOT &&
                                    item.Slot <= (int) EquipmentSlot.Bag4)
                                {
        ...
        private static void CharacterRenameCallback(IRealmClient client, CharacterRecord ch, string newName,
                                                    EntityId guid)
        {
