...
namespace WCell.RealmServer.Handlers
{
    class Asda2PvpHandler
    {
        ...
        public static void PvpRquestRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            var targetsessionid = packet.ReadUInt16();
            var bytes=  packet.ReadBytes(2);
            StringBuilder sb = new StringBuilder();
            foreach (byte b in bytes)
                sb.Append(b.ToString("X2"));
            string hexString = sb.ToString();
            //Console.WriteLine("Currect hex position: " + hexString);
            //var targetName = packet.ReadAsdaString(20, Locale.En);
            var chr = World.GetCharacterBySessionId(targetsessionid);
            client.ActiveCharacter.SendErrorMsg(string.Format("session id = {0}", targetsessionid));
        ...
        public static void AnswerPvpRequestRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            packet.Position -= 9;
            /*Console.WriteLine(packet.ToHexDump());
            var bytes = packet.ReadBytes(5);
            Console.WriteLine(BitConverter.ToString(bytes));*/
            var accepted = packet.ReadByte() == 1;//default : 1Len : 1
            //client.ActiveCharacter.SendErrorMsg(string.Format("accepted = {0}", accepted));
            if(accepted)
            {
                //start pvp
