...
namespace WCell.RealmServer.Handlers
{
    public static class FunctionalItemsHandler
    {
        ...
        {
            packet.Position -= 7;//nk8 default : 1Len : 2
            ...
            client.ActiveCharacter.SendErrorMsg(targetCharName);
            if (targetChr == null)
            {
                client.ActiveCharacter.SendErrorMsg(string.Format("{0} is not in game.", targetCharName));
                return;
            }
            ...
            if (scrollRemoved)
            {
                ...
                client.ActiveCharacter.UpdateTitleCounter((Constants.Achievements.Asda2TitleId)76, 20, 100);
            }
            else
        ...
        private static void ProcessFunctionalItem(IRealmClient client, uint parametr, short slot)
        {
            ...
            RealmServer.IOQueue.AddMessage(() =>
            {
                try
                {
                    ...
                    {
                        case Asda2ItemCategory.ReduceMpConsumpiton:
                        ...
                        case Asda2ItemCategory.IncMoveSpeed:
                                ...
                                DateTime.Now > item.Record.AuctionEndTime)
                            {
                                ...
                                //Çäåñü íóæíî óäàëèòü èòåì âèçóàëüíî â ðþêçàêå è ïî âîçìîæíîñòè íà ïàíåëè ãîðÿ÷èõ êëàâèø
                            }
                            else
                            {
                        ...
                        case Asda2ItemCategory.InstantRecover100PrcHPandMP:
                            ...
                            Asda2CharacterHandler.SendHealthUpdate(client.ActiveCharacter, true, true);
                            Asda2CharacterHandler.SendCharMpUpdateResponse(client.ActiveCharacter, true, true);
                            break;
                        case Asda2ItemCategory.InstantRecover100PrcHP:
                            ...
                            break;
                        case Asda2ItemCategory.InstantRecover100PrcMP:
                            if (client.ActiveCharacter.Last100PrcRecoveryUsed + 30000 > (uint)Environment.TickCount)
                                status = UseFunctionalItemError.CoolingTimeRemain;
                            else
                            {
                        ...
                        case Asda2ItemCategory.IncreaceWeightCapasity:
                            if (client.ActiveCharacter.WeightCapacity >= 15000)
                            {
                                return;
                            }
                            else
                            {
                                client.ActiveCharacter.WeightCapacity += 3750;
                            }
                            break;
                        case Asda2ItemCategory.RecoverHp10TimesByPrcOver30Sec:
                            ...
                            else
                            {
                                ...
                                a = new PereodicAction(client.ActiveCharacter, item.Template.ValueOnUse, 10, 3000,
                                    Asda2PereodicActionType.HpRegenPrc);
                        ...
                        case Asda2ItemCategory.ResetOneSkill:
                            ...
                            // TODO : تصليح الشجرية
                            var spells = client.ActiveCharacter.Spells.Where(x => x.ClassMask == (Asda2ClassMask)parametr) ;
                            foreach (var spell in spells)
                            {
                                ...
                                Asda2CharacterHandler.SendLearnedSkillsInfo(client.ActiveCharacter);
                                SendPreResurectResponse(client);
                                SendAllSkillsResetedInfoMsgResponse(client);
                                SendAllSkillsResetedResponse(client);
                            #region old code
                            ...
                            #endregion
                            break;
                        case Asda2ItemCategory.Points100Card:
                            client.Account.AccountData.Points += 100;
                            break;
                        case Asda2ItemCategory.Points50Card:
                            client.Account.AccountData.Points += 50;
                            break;

                        default:
                            status = UseFunctionalItemError.NotAunctionalItem;
                            client.ActiveCharacter.SendErrorMsg(string.Format("{0}", item.Category));
                            break;
                    }
                }
        ...
        public static void SendWarehouseSlotsExpandedResponse(IRealmClient client, bool isAvatar)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.WarehouseSlotsExpanded))//5456
            {
                ...
                packet.WriteInt16((isAvatar ? client.ActiveCharacter.Record.PremiumAvatarWarehouseBagsCount : client.ActiveCharacter.Record.PremiumWarehouseBagsCount + 1) * 30);//{slots}default value : 60 Len : 2
                client.Send(packet, addEnd: true);
            }
        }
        public static void SendCancelCancelFunctionalItemResponse(IRealmClient client, short itemId)
        {
            ...
            using (var packet = new RealmPacketOut(RealmServerOpCode.CancelFunctionalItem))//5454
            {
                ...
                client.ActiveCharacter.SendPacketToNear(packet, true);
            }
        }
        ...
        [PacketHandler(RealmServerOpCode.TransferUpgrade)]
        public static void TransferUpgrade(IRealmClient client, RealmPacketIn packet)
        {
            packet.Position += 3;
            var slot = packet.ReadInt16();
            packet.Position -= 10;
            var slotgem = packet.ReadInt16();
            Console.WriteLine(packet.ToHexDump() +$"\n {slot} {slotgem}");
            var item = client.ActiveCharacter.Asda2Inventory.GetShopShopItem(slot);
            var safegem = client.ActiveCharacter.Asda2Inventory.GetShopShopItem(slotgem);
            //client.ActiveCharacter.SendErrorMsg(string.Format("Slot = {0}", slot));
            if (item == null)
            {
                client.ActiveCharacter.SendErrorMsg("لم يتم ايجاد الاداة");
            }
            else if (item.EnchantChances < 1)
            {
                client.ActiveCharacter.SendErrorMsg("الاداة لا تمتلك فرص تطوير");
            }
            /*else if (item.Enchant > 9 || item.EnchantLevel > 200)
            {
                client.ActiveCharacter.SendErrorMsg("وصلت الاداة لاقصى حد تطوير سيتم ضبط التطوير تلقائيا");
            }*/
            else
            {
                if (safegem == null || safegem.Category != Asda2ItemCategory.None)
                {
                    SendUpgradeTransfered(client, UpgradeTransferStatus.Fail, item, safegem);
                    return;

                }
                item.Enchant += safegem.Enchant;
                item.EnchantLevel += safegem.EnchantLevel;
                if (item.EnchantLevel > 200)
                {
                    item.EnchantLevel = 200;
                }
                item.EnchantCount = safegem.EnchantCount;
                item.EnchantChances = safegem.EnchantChances;
                safegem.Destroy();
                Asda2InventoryHandler.ItemRemovedFromInventoryResponse(client.ActiveCharacter, safegem, DeleteOrSellItemStatus.Ok);
                Asda2InventoryHandler.UpdateItemInventoryInfo(client, item);
                SendUpgradeTransfered(client, UpgradeTransferStatus.Ok, item, safegem);
                client.ActiveCharacter.SendMoneyUpdate();
                Asda2LoginHandler.SendInventoryInfoResponse(client);
            }
        }

        public static void SendUpgradeTransfered(IRealmClient client, UpgradeTransferStatus status, Asda2Item upgradedItem, Asda2Item gem)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.UpgradeTransfered)) //6505
            {
                packet.WriteByte((byte)status); //{status}default value : 1 Len : 1
                packet.WriteInt32(client.ActiveCharacter.Asda2Inventory.Weight);//value name : unk4 default value : 0Len : 4
                Asda2InventoryHandler.WriteItemInfoToPacket(packet, upgradedItem, false);
                Asda2InventoryHandler.WriteItemInfoToPacket(packet, gem, true);
