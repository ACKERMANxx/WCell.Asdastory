...
namespace WCell.RealmServer.Handlers
{
    public class Asda2DigMgr
    {
        ...
        public static void ProcessDig(IRealmClient client)
        {
            ...
            Asda2CharacterHandler.SendEmoteResponse(client.ActiveCharacter, 110);
                Asda2DiggingHandler.SendDigEndedResponse(client,true,null);
                var templ = isUseOil
            ...
            Asda2TitleChecker.OnDig(client.ActiveCharacter);
            client.ActiveCharacter.IsDigging = true;
    ...
    public static class Asda2DiggingHandler
    {
        ...
        public static void StartDigRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            {
                SendStartDigResponseResponse(client, Asda2DigResult.DiggingFail, showel);
                return;
            }
            Asda2InventoryHandler.SendCharUsedItemResponse(Asda2_Items.UseItemResult.Ok, client.ActiveCharacter, showel);
            ...
            if (showel == null || showel.Category != Asda2ItemCategory.Showel)
            {
                SendStartDigResponseResponse(client, Asda2DigResult.YouHaveNoShowel, showel);
                return;
            }  
            var oilItem = client.ActiveCharacter.Asda2Inventory.Equipment[10];
            var isUseOil = oilItem != null && oilItem.Category == Asda2ItemCategory.DigOil;
            ...
            {
                SendStartDigResponseResponse(client, Asda2DigResult.YouUnableToDigInThisLocationDueLowLevel, showel);
                return;
            }
            ...
            SendStartDigResponseResponse(client, Asda2DigResult.Ok, showel);

        }

        public static void SendStartDigResponseResponse(IRealmClient client,Asda2DigResult status, Asda2Item showel)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.StartDigResponse))//5429
            {
                packet.WriteByte(1);//{status}default value : 3 Len : 1
                packet.WriteInt32(showel.Template.Id);
                Asda2InventoryHandler.WriteItemInfoToPacket(packet, showel, false);
                Asda2CharacterHandler.SendEmoteResponse(client.ActiveCharacter, 110, 0, 0, 0);
