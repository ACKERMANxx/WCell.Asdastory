...
namespace WCell.RealmServer.Handlers
{
    ...
    internal class  Asda2CharacterHandler
    {
        ...
        public static void RepairItemRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            //Console.WriteLine(BitConverter.ToString(packet.ReadBytes(10)));
            //packet.Position -= 9;
            var oneItemRepairSlot = 0;
            for (int i = 0; i < 40; i += 1)
            {
                //packet.Position -= 9;
                slots[i] = (short) (packet.ReadInt16());//default : 0Len : 2
            }
        ...
        public static void SendResurectWithChangeLocationResponse(IRealmClient client)
        {
            ...
            {
                packet.WriteInt16(client.ActiveCharacter.SessionId);//{sessId}default value : 53 Len : 2
                packet.WriteByte((byte)client.ActiveCharacter.MapId);//{map}default value : 3 Len : 1
                packet.WriteFixedAsciiString("",20,Locale.Any);//{ipAddr}default value :  Len : 20
                packet.WriteInt16(15604);//{port}default value : 15604 Len : 2
                packet.WriteInt16((byte)client.ActiveCharacter.BindLocation.Position.XY.X);//{x}default value : 125 Len : 2
                packet.WriteInt16((byte)client.ActiveCharacter.BindLocation.Position.XY.Y);//{y}default value : 390 Len : 2
        ...
        public static void SendCharMpUpdateResponse(Character chr, bool animate = false, bool animateGreenDights = false)
        {
            if (chr == null || chr.Map == null || chr.IsDeleted || !chr.Client.IsGameServerConnection)
                return;
        ...
        public static void ResurectOnDeathPlaceRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            Console.WriteLine(packet.ToHexDump()+"\n"+ BitConverter.ToString(packet.ReadBytes(2)));
            var toTown = packet.ReadByte();
            Console.WriteLine(toTown);
        ...
        public static void SendUpdateStatsResponse(IRealmClient client)
        {
            ...
            using (var packet = new RealmPacketOut(RealmServerOpCode.UpdateStats))//4060
            {
                ...
                packet.WriteInt32(ProcessOwerFlow((int)client.ActiveCharacter.BlockChance));//{maxBlock}default value : 0 Len : 4
                packet.WriteInt32(ProcessOwerFlow((int)client.ActiveCharacter.BlockValue));//{minBLock}default value : 0 Len : 4
                packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.Weight);//value name : unk18 default value : 15Len : 2
                packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.Weight);//value name : unk19 default value : 7Len : 2
                packet.WriteInt16(client.ActiveCharacter.Asda2Inventory.Weight);//value name : unk20 default value : 4Len : 2
        ...
        public static RealmPacketOut SelectCharacterInfo(Character selectedChr)
        {
            ...
            packet.WriteInt32(selectedChr.RealProffLevel);
            return packet;
        ...
        public static void CreateCharacterRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            if (acc == null || client.ActiveCharacter != null)
                return;
            //default : 0 accId
            packet.Position += 1; //unknown default : 0
            ...
            var name = packet.ReadString(20); //default : "",20 

            //name = "Tester" + ran.Next(999999).ToString();
            // [CBT ONLY ^] REMOVE WHEN CBT ENDED
            ...
            var outfit = packet.ReadByte();
            packet.Position += 3;
            var shirt = packet.ReadInt16();
            packet.Position += 2;
            var gloves = packet.ReadInt16();
            packet.Position += 2;
            var pants = packet.ReadInt16();
            packet.Position += 2;
            var shoes = packet.ReadInt16();
            /*Console.WriteLine(string.Format("name = {0}", name));
            Console.WriteLine(string.Format("gender = {0}", gender));
            Console.WriteLine(string.Format("hair = {0}", hair));
            Console.WriteLine(string.Format("color = {0}", color));
            Console.WriteLine(string.Format("face = {0}", face));
            Console.WriteLine(string.Format("zodiac = {0}", zodiac));
            Console.WriteLine(string.Format("outfit = {0}", outfit));
            Console.WriteLine(string.Format("shirt = {0}", shirt));
            Console.WriteLine(string.Format("gloves = {0}", gloves));
            Console.WriteLine(string.Format("pants = {0}", pants));
            Console.WriteLine(string.Format("shoes = {0}", shoes));
            Console.WriteLine(string.Format("AccId = {0}", acc.AccountId));
            Console.WriteLine(string.Format("Num = {0}", characterNum));*/
            ...
            SetUpRecords(shirt, gloves, pants, shoes, characterNum, acc, record);
            ...
            record.shirt = shirt;
            record.gloves = gloves;
            record.pants = pants;
            record.shoes = shoes;

            record.SetupNewRecord(ArchetypeMgr.GetArchetype(RaceId.Human, ClassId.NoClass), shirt, gloves, pants, shoes);
        ...
        public static void SendCreateCharacterResponseOneResponse(IRealmClient client, CharacterRecord newCharecter)
        {
            ...
            {
                PacketStructureSegments.CharacterInformation(client, packet, newCharecter);
                PacketStructureSegments.CharacterShape(client, packet, newCharecter);
                PacketStructureSegments.CharacterAbilities(client, packet, newCharecter);
                client.Send(packet, addEnd: false);
            }
        }

        public static void SetUpRecords(int shirt, int gloves, int pants, int shoes, byte characterNum, RealmAccount acc, CharacterRecord record)
        {
            Asda2ItemTemplate ShirtTemplate = Asda2ItemMgr.GetTemplate(shirt);
            Asda2ItemTemplate GlovesTemplate = Asda2ItemMgr.GetTemplate(gloves);
            Asda2ItemTemplate PantsTemplate = Asda2ItemMgr.GetTemplate(pants);
            Asda2ItemTemplate ShoesTemplate = Asda2ItemMgr.GetTemplate(shoes);

            var ShirtRec = Asda2ItemRecord.CreateRecord(ShirtTemplate);
            var GlovesRec = Asda2ItemRecord.CreateRecord(GlovesTemplate);
            var PantsRec = Asda2ItemRecord.CreateRecord(PantsTemplate);
            var ShoesRec = Asda2ItemRecord.CreateRecord(ShoesTemplate);

            var LowId = 1000000 * characterNum + acc.AccountId;

            ShirtRec.InventoryType = 3;
            ShirtRec.Slot = 1;
            ShirtRec.OwnerId = (uint)LowId;
            ShirtRec.OwnerName = record.Name;
            ShirtRec.Amount = 1;
            ShirtRec.Create();
            ShirtRec.Update();
            GlovesRec.InventoryType = 3;
            GlovesRec.Slot = 4;
            GlovesRec.OwnerId = (uint)LowId;
            GlovesRec.OwnerName = record.Name;
            GlovesRec.Amount = 1;
            GlovesRec.Create();
            GlovesRec.Update();
            PantsRec.InventoryType = 3;
            PantsRec.Slot = 2;
            PantsRec.OwnerId = (uint)LowId;
            PantsRec.OwnerName = record.Name;
            PantsRec.Amount = 1;
            PantsRec.Create();
            PantsRec.Update();
            ShoesRec.InventoryType = 3;
            ShoesRec.Slot = 3;
            ShoesRec.OwnerId = (uint)LowId;
            ShoesRec.OwnerName = record.Name;
            ShoesRec.Amount = 1;
            ShoesRec.Create();
            ShoesRec.Update();
        }
        ...
        public static void SendMySessionIdResponse(IRealmClient client)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.MySessionId))//6066
            {
                ...
                packet.WriteInt16((Int16) client.ActiveCharacter.RealProffLevel);//{profession}default value : 0 Len : 2
        ...
        public static void EmoteRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            //SendEmoteResponse(client.ActiveCharacter, 110, c, a, b);
        }
        [NotVariable]
        ...
        public static void SendUpdateAvatarMaskResponse(Character chr)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.EmoteResponse))//4025
            {
                packet.WriteInt16(chr.SessionId);//{sessId}default value : 25 Len : 2
                //packet.WriteInt32(chr.AccId);//{sessId}default value : 25 Len : 2
                packet.WriteInt16(108);//{emote}default value : 113 Len : 2
        ...
        public static void SendChangeProfessionResponse(IRealmClient client)
        {
            ...
            using (var packet = new RealmPacketOut(RealmServerOpCode.ChangeProfession))//5042
            {
                ...
                packet.WriteInt32(33554692);//{harisma}default value : 33554692 Len : 4
        ...
        public static void SendSelectFactionResResponse(IRealmClient client, SelectFactionStatus status)
        {
            ...
            if (client.ActiveCharacter.Asda2FactionId == 2)
                client.ActiveCharacter.GetTitle(Asda2TitleId.Duelist122);
        ...
        [PacketHandler((RealmServerOpCode)4301)]
        public static void SendRatesReq(IRealmClient client, RealmPacketIn packet)
        {
            SendRates(client.ActiveCharacter, 2, 2);
        }
