...
namespace WCell.RealmServer.Handlers
{
    ...
    internal class Asda2InventoryHandler
    {
                                                    ...
                                                    int destQuant = 0, short destWeight = 0, bool secondItemIsNullNow = false)
        {
            ...
            {
                packet.WriteByte((byte)status); //{status}default value : 1 Len : 1 0 - нет места . 1 - ok, 2 - недостаточно инфы о предмете - 3 - предмен не предназначен для ношения
        ...
        public static void SendCharacterRemoveEquipmentResponse(Character chr, short slot, int itemId)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.CharacterRemoveEquipment)) //4023
            {
               ...
               // packet.WriteInt32(chr.AccId);
                packet.WriteInt16(slot); //{slot}default value : 9 Len : 2
                packet.WriteInt32(-1); //{ItemId}default value : 21499 Len : 4
                packet.WriteInt32(0);
        ...
        public static void UseItemRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            var chr = client.ActiveCharacter;
            var inv = packet.ReadByte(); //default : 2Len : 1
            var slot = packet.ReadInt16(); //default : 39Len : 4
            if (inv != 2 || slot < 0 || slot > 69)
            {
                chr.YouAreFuckingCheater("Trying to use item from wrong slot.", 20);
                chr.SendErrorMsg(string.Format("inv = {0}, Slot = {1}", inv, slot));
                return;
            }
            chr.Asda2Inventory.UseItem((Asda2InventoryType)inv, (byte)slot);
       ...
       //here4
        ...
        public static void BuyItemRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            for (int i = 0; i < 5; i += 1)
            {
                var itemId = packet.ReadInt32(); //default : 1043Len : 2
                //var isEquipment = packet.ReadInt16(); //default : 1Len : 2
                var quantity = packet.ReadInt16(); //default : -1Len : 2
                client.ActiveCharacter.SendErrorMsg($"{i} > {itemId} : {quantity}");
                if (quantity == -1)
                    quantity = 1;
        ...
        #endregion
        //Done
        #region OptionTransfer
        [PacketHandler(RealmServerOpCode.OptionTransfer)]
        public static void OptionTransferRequest(IRealmClient client, RealmPacketIn packet)
        {
            packet.Position -= 9;
            var item1slot = packet.ReadInt16();
            packet.Position += 9;
            var item2slot = packet.ReadInt16();
            packet.Position += 4;
            var boostItem = packet.ReadInt32();
            var cost = 0;
            
            var item1 = client.ActiveCharacter.Asda2Inventory.ShopItems[item1slot];
            var item2 = client.ActiveCharacter.Asda2Inventory.ShopItems[item2slot];

            if (item1.RequiredLevel >= 1 && item1.RequiredLevel < 5)
                cost = 500;
            if (item1.RequiredLevel >= 5 && item1.RequiredLevel < 10)
                cost = 3000;
            if (item1.RequiredLevel >= 10 && item1.RequiredLevel < 20)
                cost = 6000;
            if (item1.RequiredLevel >= 20 && item1.RequiredLevel < 30)
                cost = 24000;
            if (item1.RequiredLevel >= 30 && item1.RequiredLevel < 40)
                cost = 48000;
            if (item1.RequiredLevel >= 40 && item1.RequiredLevel < 60)
                cost = 120000;
            if (item1.RequiredLevel >= 60)
                cost = 240000;

            item2.Parametr1Type = item1.Parametr1Type;
            item2.Parametr1Value = item1.Parametr1Value;
            item2.Parametr2Type = item1.Parametr2Type;
            item2.Parametr2Value = item1.Parametr2Value;
            if (item1.Parametr3Type != Asda2ItemBonusType.None)
            {
                item2.Parametr3Type = item1.Parametr3Type;
                item2.Parametr3Value = item1.Parametr3Value;
            }
            if (item1.Parametr4Type != Asda2ItemBonusType.None)
            {
                item2.Parametr4Type = item1.Parametr4Type;
                item2.Parametr4Value = item1.Parametr4Value;
            }
            if (item1.Parametr5Type != Asda2ItemBonusType.None)
            {
                item2.Parametr5Type = item1.Parametr5Type;
                item2.Parametr5Value = item1.Parametr5Value;
            }
            client.ActiveCharacter.SubtractMoney((uint)cost);
            item1.Destroy();
            ItemRemovedFromInventoryResponse(client.ActiveCharacter, item1, DeleteOrSellItemStatus.Ok);
            UpdateItemInventoryInfo(client, item2);
            SendOptionTransferResponse(client, item2);
        }

        public static void SendOptionTransferResponse(IRealmClient client, Asda2Item item)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.OptionTransfered))
            {
                packet.WriteByte(1);
                packet.WriteByte(1);
                WriteItemInfoToPacket(packet, item);
        ...
        #endregion
        //Done
        #region ResetEnchantChances
        [PacketHandler(RealmServerOpCode.FixGearRequest)]
        public static void ResetEnchantChancesRequest(IRealmClient client, RealmPacketIn packet)
        {
            packet.Position -= 5;
            var itemSlot = packet.ReadInt16();
            packet.Position += 10;
            var scrollSlot = packet.ReadInt16();
            packet.Position -= 16;
            var type = packet.ReadByte();
            var item = client.ActiveCharacter.Asda2Inventory.ShopItems[itemSlot];
            var scroll = scrollSlot == -1 ? null : client.ActiveCharacter.Asda2Inventory.ShopItems[scrollSlot];
            var cost = 0;

            if (item.RequiredLevel >= 1 && item.RequiredLevel < 20)
                cost = 3600;
            if (item.RequiredLevel >= 20 && item.RequiredLevel < 40)
                cost = 43200;
            if (item.RequiredLevel >= 40 && item.RequiredLevel < 60)
                cost = 144000;
            ...
            if (type == 0)
            {
                ...
                SendGearFixed(client, item, scroll);
            }
            else
            {
                item.EnchantChances = item.FailedEnchants;
                item.Enchant = item.Enchant;
                item.EnchantCount = item.Enchant;
                item.FailedEnchants = 0;
                if (item.RequiredLevel < 20)
                {
                    scroll.ModAmount(-1);
                }
                else if (item.RequiredLevel > 19 && item.RequiredLevel < 40)
                {
                    scroll.ModAmount(-2);
                }
                else if (item.RequiredLevel > 39 && item.RequiredLevel < 60)
                {
                    scroll.ModAmount(-3);
                }
                else
        ...
        public static void SendGearFixed(IRealmClient client, Asda2Item item, Asda2Item scroll)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.FixGearResponse))
            {
                packet.WriteByte(1);
        ...
        public static void MoveSoulStoneInRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.Asda2Inventory.SowelItem(itemSlot, sowelCell, sowelSlot);
        }

        public static void SendItemSoweledResponse(IRealmClient client, int inventoryWeight, SowelingStatus status, Asda2Item item, Asda2Item stone)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.ItemSoweled)) //5025
            {
                packet.WriteByte((byte)status); //{status}default value : 1 Len : 1
                PacketStructureSegments.ItemInformation(null, packet, item , true);
                PacketStructureSegments.ItemInformation(null, packet, stone, true);
                //WriteItemInfoToPacket(packet, item, false);
                //WriteItemInfoToPacket(packet, stone, false);
                client.Send(packet, addEnd: false);
            }
        ...
        {
            PacketStructureSegments.ItemInformation(null, packet, item);
        }
        ...
        public static void MoveSoulStoneOutRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.SendErrorMsg(string.Format("sowelNum = {0}", sowelNum));
            client.ActiveCharacter.SendErrorMsg(string.Format("itemSlot = {0}", itemSlot));
            ...
            switch (sowelNum)
            {
                case 1:
                    ...
                    client.ActiveCharacter.Asda2Inventory.TryAdd(item.Soul1Id, 1, true,
                                                                 ref newSowel);
                    item.Soul1Id = 0;
                    UpdateItemInventoryInfo(client, item);
                    break;
            ...
            Log.Create(Log.Types.ItemOperations, LogSourceType.Character, client.ActiveCharacter.EntryId)
                                                 ...
                                                 .AddReference(srcLog)
                                                 .Write();
            SendSowelRemovedResponse(client, SowelRemovedStatus.Ok, item, newSowel);
            UpdateItemInventoryInfo(client, item);


        }
        public static void SendSowelRemovedResponse(IRealmClient client, SowelRemovedStatus status, Asda2Item mainItem = null, Asda2Item sowel = null)
        {
        ...
        public static void OpenBoosterRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            //client.ActiveCharacter.SendErrorMsg(string.Format("BoosterId = {1} * Inv = {1} * Cell = {2}", boosterId, inv, cell));
        ...
        public static void SendbosterOpenedResponse(IRealmClient client, OpenBosterStatus status, Asda2Item addedItem, Asda2InventoryType boosterInv, short boosterCell, Int16 weight)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.bosterOpened)) //5469
            {
                ...
                if (addedItem.Template.Quality == Asda2ItemQuality.Purple || addedItem.Template.Quality == Asda2ItemQuality.Green)
                packet.WriteInt16(11); //value name : unk4 default value : 11Len : 2
                else
                packet.WriteInt16(0); //value name : unk4 default value : 11Len : 2
        ...
        {
            packet.Position -= 2; //tab36 default : stab36Len : 1
            var packageInv = (Asda2InventoryType)packet.ReadByte(); //default : 1Len : 1
            var packageSlot = packet.ReadInt16(); //default : 6Len : 2
            client.ActiveCharacter.SendErrorMsg(string.Format("Inv = {0} / Slot = {1}", packageInv, packageSlot));
        ...
        public static void DisasembleEquipmentRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            //client.ActiveCharacter.SendErrorMsg(string.Format("itemId = {0}", itemId));
        ...
        public static void ByuItemFromWarshopRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.SendErrorMsg("Id = " + internalWarShopId);
        ...
        public static void AdvacedEnchantWeaponRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            var item = client.ActiveCharacter.Asda2Inventory.GetShopShopItem(itemSlot);
            //client.ActiveCharacter.SendErrorMsg(string.Format("Slot = {0}", itemSlot));
            if (item == null)
            {
                client.ActiveCharacter.SendInfoMsg("Item not found. Restart game please.");
            ...
            if (item.Template.Quality != Asda2ItemQuality.Green && item.Template.Quality != Asda2ItemQuality.Purple)
            {
            ...
            {
                if (res1.ItemId != 20959 || res2.ItemId != 20926 || res3.ItemId != 20961)
                {
            ...
            switch (item.Template.AuctionLevelCriterion)
            {
                case AuctionLevelCriterion.Zero:
                    if (res1.Amount < 10 || res2.Amount < 30 || res3.Amount < 14)
                    {
        ...
        public static void CombineItemRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.SendErrorMsg(string.Format("ComId = {0}", comtinationId));
        ...
        public static void SendItemMailListResponse(IRealmClient client, List<Asda2DonationItem> items)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.ItemMailList))//11005
            {
                ...
                for (int i = 0; i < 7; i += 1)
                {
                    ...
                    packet.WriteByte(1);//{slot}default value : 3 Len : 1
        ...
        [PacketHandler(RealmServerOpCode.RefreshPoints)]//5466
        public static void RefreshPointsRequest(IRealmClient client, RealmPacketIn packet)
        {
            var chr = client.ActiveCharacter;
            var records = Orders.GetOrders((int)chr.AccId);
            Asda2Item item = new Asda2Item();
            for (int i = 0; i < records.Length; i++)
            {
                var rec = records[i];
                if (chr.Name == rec.CharacterName)
                {
                    chr.Asda2Inventory.TryAdd(rec.ItemId, rec.Amount, false, ref item, (Asda2InventoryType)Asda2ItemMgr.GetTemplate(rec.ItemId).InventoryType);
                    
                    rec.DeleteAndFlush();
                }
            }
            SendPointsRefreshedResponse(client);
        }

        public static void SendPointsRefreshedResponse(IRealmClient client)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.PointsRefreshed))//5467
            {
                packet.WriteByte(1);//{result}default value : 1 Len : 1
                packet.WriteInt32(client.Account.AccountData.Points);//value name : unk4 default value : 0Len : 4 // Points
                client.Send(packet, addEnd: true);
            }
        }
    ...
    {
        Ok = 1,
        NoMoreUpgradeChances = 2,
        YouDidntUseMinmumResets = 3,
        NotAnExtrenalItem = 4,
        UpgradeFail = 5,
        NoLevelMatch = 6,
        YouCantUpgradeThisItem = 7,
        Break = 8
    }

    public enum UpgradeTransferStatus
    {
        Ok = 1,
        Fail = 2
    }
