...
namespace WCell.RealmServer.Handlers
{
    public class Asda2TradeWindow
    {
        ...
        Asda2PushItemToTradeStatus SetCharacterItemRefs(Character character, Asda2ItemTradeRef itemRef)
        {
            ...
            {
                character.SendErrorMsg(string.Format("itemRef {0} / itemRef Amount {1} / itemRef itemAmount {2}", itemRef.Item, itemRef.Amount, itemRef.Item.Amount));
                character.SendErrorMsg(string.Format("itemRef ItemId {0}", itemRef.Item.ItemId));
        ...
        public Asda2PushItemToTradeStatus PushItemToTrade(Character activeCharacter, short cellNum, int quantity, byte invNum, ref Asda2ItemTradeRef item)
        {
            ...
            {
                activeCharacter.SendErrorMsg(string.Format("CellNum == {0} * InvNum == {1} * Quantity == {2}", cellNum, invNum, quantity));
        ...
        private void TransferItems()
        {
            ...
            if (TradeType == Asda2TradeType.RedularTrade)
            {
                if (SecondChar.Asda2Inventory.FreeRegularSlotsCount < firstCharItemsCount || FisrtChar.Asda2Inventory.RegularItems.Count(i => i == null) < secondCharItemsCount)
                {
            ...
            {
                if (SecondChar.Asda2Inventory.ShopItems.Count(i => i == null) < firstCharItemsCount || FisrtChar.Asda2Inventory.ShopItems.Count(i => i == null) < secondCharItemsCount)
                {
                    FisrtChar.SendSystemMessage("Check free space in inventory!.");
                    SecondChar.SendSystemMessage("Check free space in inventory!.");
                    CancelTrade();
