...
namespace WCell.RealmServer.Handlers
{
    public static class Asda2PrivateShopHandler
    {
        ...
        public static void OpenPrivateShopRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            packet.Position -= 4; //nk8 default : 4Len : 1
                                                         ...
                                                         Asda2ItemTradeRef[] items)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.PrivateShopOpened)) //6303
            {
                ...
                if (status == PrivateShopOpenedResult.Ok)
                {
                    ...
                    packet.WriteByte(validitemscount); //value name : unk6 default value : 4Len : 1
                    for (int i = 0; i < validitemscount; i += 1)
                    {
                        var item = items == null || items[i] == null || items[i].Item == null ? null : items[i].Item;
                        if (item == null)
                            continue;
                        packet.WriteInt32(item == null ? -1 : item.ItemId); //{itemId}default value : 13387 Len : 4
                        ...
                        packet.WriteInt32(items == null || items[i] == null ? -1 : items[i].Amount);
                        ...
                        packet.WriteInt32(items == null || items[i] == null ? -1 : items[i].Price);
                                                                ...
                                                                Asda2PrivateShop shop)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.CharacterPrivateShopInfo)) //6308
            {
                ...
                if (infoStatus == Asda2ViewTradeShopInfoStatus.Ok)
                {
                    ...
                    for (int i = 0; i < shop.ItemsCount; i += 1)
                    {
                        var item = shop.ItemsOnTrade[i] == null || shop.ItemsOnTrade[i].Item == null
                                       ? null
                                       : shop.ItemsOnTrade[i].Item;
                        packet.WriteInt32(item == null ? -1 : item.ItemId); //{itemId%}default value : 0 Len : 4
                        ...
                        packet.WriteInt16(item == null ? 0 : item.EnchantLevel); //{enchant%}default value : 0 Len : 2
        ...
        public static void PrivateShopChatReqRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.SendErrorMsg(string.Format("PP = {0}", packet.Position));
            client.ActiveCharacter.SendErrorMsg(string.Format("msg = {0}", msg));
            if (client.ActiveCharacter.PrivateShop == null)
            {
        ...
        public static void BuyItemFromCharacterPrivateShopRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            for (int i = 0; i < 6 && packet.ReadInt32() != 0; i += 1)
            {
                ...
                //client.ActiveCharacter.SendErrorMsg(string.Format("amount = {0}", amount));
    ...
    public class Asda2PrivateShop
    {
        ...
        public PrivateShopOpenedResult StartTrade(List<Asda2ItemTradeRef> itemsToTrade, string title)
        {
            ...
            if (Owner.EliteShopBannerEnabled)
            {
                Owner.IsAsda2TradeDescriptionPremium = true;
                Owner.SendPacketToArea(Chat.ChatMgr.SendAnnChatResponse(Owner.Client, Owner.Name, string.Format("{0}، [ X : {1} | Y : {2} ]", title, Owner.Position.X, Owner.Position.Y), Locale.Any), true, true, Locale.Any, 50f);
                var lastSend = DateTime.Now;
                var timeToSend = lastSend + TimeSpan.FromSeconds(5);
            ...
            Owner.Map.AddMessage(() =>
                                     {
                                         ...
                                         foreach (var asda2ItemTradeRef in itemsToBy)
                                         {
                                             ...
                                             if (asda2ItemTradeRef.Amount >= asda2ItemTradeRef.Item.Amount)
                                             {
                                                 ...
                                                 activeCharacter.Asda2Inventory.TryAdd(item.ItemId,item.Amount,true,ref addedItem, new Asda2InventoryType?(), item);
                                             ...
                                             else
                                             {
                                                 ...
                                                 activeCharacter.Asda2Inventory.TryAdd((int)asda2ItemTradeRef.Item.Template.ItemId, asda2ItemTradeRef.Amount, true, ref newItem,new Asda2InventoryType?(),asda2ItemTradeRef.Item);
                                                 Log.Create(Log.Types.ItemOperations, LogSourceType.Character, activeCharacter.EntryId)
