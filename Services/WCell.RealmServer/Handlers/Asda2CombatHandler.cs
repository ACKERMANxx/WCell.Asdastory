...
namespace WCell.RealmServer.Handlers
{
    class Asda2CombatHandler
    {
        ...
        /// <param name="status">0 - stop;1-start;2 90% weight;3 cannot see target;5 cannot attack target;6 canot attack now;7 no arrows;8 canot attack on vechiel;9 11 12 cant attak from this place;20 لا تستطيع الهجوم علي الوحش اذا كان مستواك اعلي منه ب 12 مستوي</param>
        public static void StartAtackResponse(Character chr,Unit target, byte status)
        {
            ...
            /*using (var p = new RealmPacketOut(RealmServerOpCode.StartAtackResponse)) //4027
            {
                ...
                p.WriteInt16(0);
                p.WriteInt16(0);
                p.WriteInt32(npc == null ? -1 : npc.Health); //{UniqMonstrId}default value : 40 Len : 4
                p.WriteInt32(-1);
                p.WriteInt32(-1);
                p.WriteInt32(-1);
                p.WriteInt32(-1);
        ...
        public static void SendAttackerStateUpdate(DamageAction action)
        {
            if(action.Attacker is Character && action.Victim is NPC)
            {
                ...
                using (var packet = new RealmPacketOut(RealmServerOpCode.MonstrTakeDmg))//4029
                {
                    ...
                    packet.WriteInt32(victim.Health);//value name : unk6 default value : 0Len : 4
                    if (action.VictimState == VictimState.Evade)
                        ...
                        packet.WriteUInt32(action.ActualDamage /*+ (action.IsCritical?2147483648:0)*/);//{AmountOfDmg}default value : 482 Len : 4
                    packet.WriteInt32(action.ActualDamage); //DMG
                    packet.WriteInt32(action.ActualDamage);//DMG
                    packet.WriteInt32(action.ActualDamage);//DMG
                    packet.WriteByte(0); //Type ---- 0-Start 1-the Target Is so Far. 2-I think like Start 3-you can't use spell because abnormal thing 4- you haven't Enough Arrows 5-You Can't Attack target 6-you can't Harm monster now 7-haven't enough arrow
                    packet.WriteInt32(-1); //DMG
                    packet.WriteInt32(-1);//DMG
                    packet.WriteInt32(-1);//DMG
                    packet.WriteInt32(-1);//DMG
                    packet.WriteInt32(-1); //DMG
