...
namespace WCell.RealmServer.Handlers
{
    class Asda2MovmentHandler
    {
        ...
        {
            packet.Position -= 12;//nk6 default : 1000Len : 8
        ...
        {
           /* if (chr.GodMode && !chr.IsGMVisible)
            {
                ...
                using (var packet = new RealmPacketOut(RealmServerOpCode.StartMoveCommon))//4007
                {
                    ...
                    packet.WriteFloat(1 * 100); //default value : 1
                    packet.WriteFloat(1 * 100); //default value : 1
                    packet.WriteFloat(1 * 100); //default value : 1
                    packet.WriteFloat(1 * 100); //default value : 1
        ...
        public static void StartMoveRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            try
            {
                //packet.Position += 1;//default : md5Len : 16
                packet.Position -= 12;
            ...
            if (chr.IsAsda2BattlegroundInProgress &&
                (!chr.CurrentBattleGround.IsStarted &&
                 new Vector3(chr.Map.Offset + x1, chr.Map.Offset + y1, 0).GetDistance(chr.CurrentBattleGround.GetBasePosition(chr)) > 20 ||
                 new Vector3(chr.Map.Offset + x1, chr.Map.Offset + y1, 0).GetDistance(chr.CurrentBattleGround.GetForeigLocation(chr)) < 20))
            {
                SendStartComonMovePacketError(client.ActiveCharacter, false,
                                              Asda2StartMovementStatus.CantMoveBeforeWarStarted);
                return;
            }
        ...
        public static void SendStartMoveCommonToAreaResponse(Character chr, bool instant, bool includeSelf = true)
        {
            /*if (chr.GodMode && !chr.IsGMVisible)
            {
                using (var packet = new RealmPacketOut(RealmServerOpCode.StartMoveCommon)) //4007
                {
                    ...
                    chr.SendPacketToArea(packet, false);
                }
