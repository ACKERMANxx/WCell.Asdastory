	...
	public partial class SpellCast : IUpdatable, IWorldLocation
	{
		...
		internal SpellFailedReason Start(Spell spell,Unit target)
		{
			if (IsCasting)
			{
				if (!IsChanneling)
				{
					SpellHandler.SendCastFailed(Client, spell, SpellFailedReason.SpellInProgress);
		...
		private SpellFailedReason Start()
		{
            ...
            {
                InitialTargets = Spell.IsAreaSpell ? null : new WorldObject[] { SelectedTarget };
            }
            var reason = Prepare();
		...
		{
			return Start(spell, passiveCast, new []{SelectedTarget});
		}
		...
		private SpellFailedReason Prepare()
		{
			...
			try
			{
			    ...
			    m_castDelay = 0;//CasterUnit is NPC?(int)Spell.CastDelay: 0;// (int)Spell.CastDelay;
                var ct = DateTime.Now.AddMilliseconds((int)Spell.CastDelay);
                if (CasterUnit != null&&ct > CasterUnit.CastingTill)
			...
			if (reason != SpellFailedReason.Ok)
			{
				...
				if (!IsPassive && Spell.IsVisibleToClient)
				{
                    if(reason!=SpellFailedReason.OutOfRange && CasterChar!=null)
                        CasterChar.SendSystemMessage(string.Format("Cast spell failed cause : {0}",reason));
		...
		public void SendPacketToArea(RealmPacketOut packet)
		{
			...
			{
				CasterObject.SendPacketToArea(packet, true, false);
			}
			else
			{
