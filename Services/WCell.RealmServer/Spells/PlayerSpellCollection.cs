	...
	public class PlayerSpellCollection : SpellCollection
	{
		...
		void AddSpell(Spell spell, bool sendPacket)
		{
            ...
            if (m_sendPackets && sendPacket)
            {
                ...
                /*if (!spell.IsPassive)
                {
                    SpellHandler.SendVisual(Owner.Target, spell.Visual);	// ouchy: Unnamed constants 
                }*/
            }
		}
		#endregion
        ...
        public SkillLearnStatus TryLearnSpell(short skillId, byte level)
        {
			...
			/*if (level == 1)
				spell = SpellHandler.Get((uint)(skillId + 100000));*/
			if (spell == null || level <= 0)
                return SkillLearnStatus.FailedToUseSkillPoints;
            if (spell.LearnLevel > OwnerChar.Level)
                return SkillLearnStatus.YouCantLevelUpThisSkill;
            if (AvalibleSkillPoints <= 0)
                return SkillLearnStatus.NoEnoughSpechialSkillPoints;
            if (level > 0)
            {
				...
				{
					Console.WriteLine("fail" + previusLvlSpell.Level + " " + spell.Level);
					return SkillLearnStatus.YouCantLevelUpThisSkill;
				}
            ...
            AddSpell(spell, true);
            Remove((uint)spell.RealId);
            OwnerChar.SendMoneyUpdate();
            OwnerChar.PointsSpent++;
            return SkillLearnStatus.Ok;
        }
    }
}