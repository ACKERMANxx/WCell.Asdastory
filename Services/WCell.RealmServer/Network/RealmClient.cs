...
namespace WCell.RealmServer.Network
{
    ...
    public sealed class RealmClient : ClientBase, IRealmClient
    {
        ...
        protected override bool OnReceive()
        {
            ...
            {
                // var cipher = new ARC4();
                //var cipher = new Asda2Cipher();
                var receivedBuffer = _bufferSegment.Buffer.Array;
                var bufferLength = BitConverter.ToInt16(receivedBuffer, _bufferSegment.Offset + 1);
                if (_remainingLength < bufferLength)
                {
                    Console.WriteLine("canceled");
                    return true;
                }

                //Console.WriteLine("R>>>>>>>" + bufferLength.ToString());
                try
                {

                    Asda1Cipher.CryptPacket(receivedBuffer, _bufferSegment.Offset);
                }
                catch
                { }

                var packet = new byte[bufferLength];
                Buffer.BlockCopy(receivedBuffer, _bufferSegment.Offset, packet, 0, bufferLength);
                var newseg = BufferSegment.CreateSegment(packet);
                if (packet[0] == 0xFB)
                {
                    if (packet[bufferLength - 1] != 0xFE)
                    {
                        Console.WriteLine("SomethingWrong In Packet");
                        return true;
                    }
                }
                else
                ...
                //         var realmPacket = new RealmPacketIn(_bufferSegment[packet , bufferLength - 4, packetType], 7, bufferLength - 8, IsGameServerConnection);
                var realmPacket = new RealmPacketIn(newseg, 7, bufferLength - 8, IsGameServerConnection);
                //Console.WriteLine(@"S <- C: {0}", realmPacket.PacketId);

                if (realmPacket.PacketId.RawId != 1000)
                {
                }

                RealmPacketMgr.Instance.HandlePacket(this, realmPacket);

                _remainingLength -= bufferLength;

                var curBs = _bufferSegment;
                _bufferSegment = Buffers.CheckOut();
                if (_remainingLength > 0)
                {
                   // Console.WriteLine("Copied remaining " + _remainingLength);

                    Array.Copy(curBs.Buffer.Array, curBs.Offset + bufferLength, _bufferSegment.Buffer.Array,
