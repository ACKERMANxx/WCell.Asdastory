		...
		public static void SayYellEmote(this Character sender, ChatMsgType type, ChatLanguage language, string msg, float radius)
		{
			...
			if (GlobalChat)
			{
				using (var packetOut = CreateCharChatMessage(type, language, sender.EntityId, sender.EntityId, null, msg, sender.ChatTag))
				{
					...
					{
						chr.Send(packetOut, addEnd: false);
					}
				}
			}
			else
				...
				Func<WorldObject, bool> iterator = obj =>
				{
					if ((obj is Character))
					{
						...
						else
						{
							...
							chr.Send(scrambledPckt, addEnd: false);
						}
					}
					return true;
				};
...
/*
		...
		private static void WhisperParser(Character sender, ChatMsgType type, ChatLanguage language, RealmPacketIn packet)
		{
			...
			if (targetChr.IsIgnoring(sender))
			{
				...
				{
					sender.Send(packetOut, addEnd: false);
				}
			}
			else
			{
				...
				{
					targetChr.Send(packetOut, addEnd: false);
				}
			}
		...
		{
		    using (var p =CreateGlobalChatMessage("~Server~",message,Color.DodgerBlue, target.Locale))
		    {
        ...
        {
            using (var p = CreateGlobalChatMessage(sender, message, c, target.Locale))
            {
                target.Send(p, addEnd: false);
            }
        }
        ...
        public static void SendSystemMessage(this IEnumerable<Character> targets, string message)
		{
		    ...
		    foreach (var target in targets)
		    {
		        ...
		        using (
		            var packet = CreateGlobalChatMessage("~Server~", message, Color.DodgerBlue, target.Client.Locale)
		            )
		        {
		            target.Send(packet, addEnd: false);
		        }
		    }
		}
