    ...
    public static class AuthenticationHandler
    {
        ...
        public static void AuthChallengeRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            if (packet.Length <= 61)
                return;
            packet.Position += 15;
            var login = packet.ReadString(61);
            //packet.Position += 31 - login.Length;

            //if (client.Locale == Locale.Ru)
            //    packet.Position += 19;
            var password = packet.ReadString(49);

            s_log.Debug(string.Format("{0} starting login chalange, pass {1}", login, password));

            client.AccountName = login;
            client.Password = password.Substring(0, 32);
        ...
        public static string GetMD5String(string Target)
        {
            ...
            {
                byte[] DataTarget = MD5Calc.ComputeHash(Encoding.Default.GetBytes(Target));
                var StringBuild = new StringBuilder();
                for (int i = 0; i < 16; i++)
                {
                    StringBuild.Append(DataTarget[i].ToString("x2"));
                }
            ...
            else
            {
                var acctQuery = new Action(() =>
                {
                    ...
                    else
                    {
                        ...
                        if (client.Password.ToLower() != GetMD5String(acc.Password).ToLower())
                        {
        ...
        private static void QueryAccountCallback(IRealmClient client, Account acct)
        {
            ...
            if (acct == null)
            {
                ...
                Console.WriteLine(client.AccountName);


                // Account doesn't exist yet -> Check for auto creation
                if (RealmServerConfiguration.AutocreateAccounts)
                {
                    if (!AccountMgr.NameValidator(ref accName) || client.Password == null)                   {
                        OnLoginError(client, AccountStatus.WrongLoginOrPass);
                        return;
                    }
        ...
        public static void SendAuthChallengeSuccessReply(IRealmClient client)
        {
            ...
            {
                packet.WriteByte((byte)AccountStatus.Success);//status 0
                packet.WriteByte((byte)0);//Later //1
                packet.WriteInt32(client.AuthAccount.AccountId);//unknown  2-3-4-5       
                packet.WriteByte(1);//Later //6      
                packet.WriteByte(2);//Junk 7
                client.Send(packet, addEnd: false);
            }
            using (var packet = new RealmPacketOut(RealmServerOpCode.ChanelInfoResponse))//1013
            {
                //packet.WriteByte(0xF0);// unknown
                //packet.WriteInt16(0);//unknown
                packet.WriteInt16(-1);//chanel1
                client.Send(packet, addEnd: false);
            }
            //using (var packet = new RealmPacketOut(RealmServerOpCode.ChanelInfoResponse))//1013
            //{
        ...
        public static void SendCharacterNamesResponse(IRealmClient client)
        {
            ...
            using (var packet = new RealmPacketOut(RealmServerOpCode.CharacterNames))//1006
            {
                ...
                foreach (var character in client.AuthAccount.Characters)
                {
                    ...
                    packet.WriteString(character.Name, 21);//default value : "",21
                    ...
                    packet.WriteInt64(0L);//value name : _
        ...
        public static void SendCharacterInfoLSResponse(IRealmClient client)
        {
            ...
            foreach (var character in client.Account.Characters)
            {
                using (var packet = new RealmPacketOut(RealmServerOpCode.CharacterInfoLS)) //1007
                {
                    ...
                    packet.WriteInt32(0); //default value : 0 Junk
                    packet.WriteByte(character.CharNum); //default value : 0
                    packet.WriteByte(2); //value name : _ Junk
                    for (int i = 0; i < 11; i++)
                    {
                        var chr = character.CreateCharacter();
                        chr.LoadSometypes(client.Account, character, client);
                        var item = chr.Asda2Inventory.Equipment[i];
                        packet.WriteInt32(item == null ? 0 : item.ItemId); //default value : 0
                        packet.WriteInt16(item == null ? -1 : item.Slot); //value name : _slot
        ...
        public static void PingRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.LastPingTime = DateTime.Now;
        }
