...
namespace WCell.RealmServer.Groups
{
	...
	public abstract partial class Group : IInstanceHolderSet, ICharacterSet, IEnumerable<GroupMember>
	{
		...
		protected Group(Character leader, byte maxGroupUnits)
		{
			...
			var member = AddMember(leader, true);
			Leader = member;
			m_masterLooter = null;
		...
		protected virtual void OnAddMember(GroupMember member)
		{
			...
			if (chr != null)
			{
                ...
                chr.Group.SendUpdate();
                member.Group.SendUpdate();
			}
		...
		public virtual void SendUpdate()
		{
			if (m_leader == null)
			{
                ...
                {
                    Asda2GroupHandler.SendPartyMemberInitialInfoResponse(member.Character);
                    Asda2GroupHandler.SendPartyMemberPositionInfoResponse(member.Character);
                }
		    /*using (syncLock.EnterReadLock())
			{
				Character chr;
		...
		public virtual void SendTargetIconList(Character requester)
		{
			using (var packet = new RealmPacketOut(RealmServerOpCode.MSG_RAID_TARGET_UPDATE))
			{
				...
				if (requester != null)
				{
					requester.Client.Send(packet, addEnd: false);
				}
				else
				{
					SendAll(packet);
			...
			ICollection<Asda2LooterEntry> looters)
		{
			...
			foreach (Character chr in center.GetObjectsInRadius(Asda2LootMgr.LootRadius, ObjectTypes.Player, false, 0))
			{
