...
namespace WCell.RealmServer.Global
{
    ...
    public partial class Map : IGenericChatTarget, IMapId, IContextHandler, IWorldSpace
    {
        ...
        public int CurrentThreadId { get { return m_currentThreadId; } }
        ...
        protected Map()
        {
            ...
            for (var l = Locale.Start; l < Locale.End; l++)
            {
                CharactersByLocale.Add(l, new List<Character>());
            }
        ...
        {
            var ltd = Loots.Where(asda2Loot => (DateTime.Now - asda2Loot.SpawnTime).TotalMinutes > CharacterFormulas.DropLiveMinutes).ToList();
            foreach (Asda2Loot asda2Loot in ltd)
            {
                foreach (var asda2LootItem in asda2Loot.Items)
                {
            ...
            /*var iLen = _lootPositions.GetLength(0);
            var jLen = _lootPositions.GetLength(1);*/
            /*for (int i = 0; i < iLen; i++)
            {
                for (int j = 0; j < jLen; j++)
                {
                    //if (_lootPositions[i, j] != null)
                    //{
                        //LogUtil.WarnException("Uncleared loot cell [{0},{1}]", i, j);
                        _lootPositions[i, j] = null;
                    //}
                }
            }*/
            CallDelayed(600000, ClearLoot);
            ...
            this()
        {
            ...
            Offset = ((float)rgnTemplate.Id) * 1000;
        ...
        {
            var templates = Asda2PortalMgr.GetSpawnPoolTemplatesByMap(MapId);
        ...
        private void MapUpdateCallback(object state)
        {
            ...
            //lock (m_mapTime)
            {
                ...
                foreach (var obj in m_objects.Values)
                {
                    ...
                    if (!obj.IsAreaActive && DefenceTownEvent == null)
                    {
                    ...
                    try
                    {
                        ...
                        if (objUpdateDelta >= minObjUpdateDelta)
                        {
                            ...
                            objCountUpdated++;
                            var delta = Environment.TickCount - startTicks;
                            if (maxObjUpdateTime < delta)
                            {
                                maxObjUpdateTime = delta;
                                if (totalUpdates > 500 && maxObjUpdateTime > 1000)
                                {
                                    totalUpdates = 0;
                                    maxObjUpdateTime = 100;
                                    LogUtil.WarnException("Object {0} updates {1} !!!", obj, delta);
                                }
                            }
                ...
                if (_avgUpdateTime > 1000)
                    LogUtil.WarnException("---=---end delta :{0}, avg :{1}, objs :{2}, objUpd :{3}, maxUpdTime: {4}, map: {5}", newUpdateDelta, _avgUpdateTime, ObjectCount, objCountUpdated, maxObjUpdateTime, Name);
        ...
        Dictionary<ushort, WorldObject> _asda2Npcs = new Dictionary<ushort, WorldObject>();
        ...
        {
            return (_asda2Npcs.ContainsKey(id) ? _asda2Npcs[id] : null) as NPC;
        }
        ...
        public void RemoveObjectNow(WorldObject obj)
        {
            ...
            if (npc != null)
            {
                ...
                Asda2CombatHandler.SendMonstrStateChangedResponse(npc, Asda2NpcState.Dead);
            }
            else
            {
            ...
            else
            {
                ...
                if (oldMap != null)
                {
                    var moveTask = new Message(() =>
                    {
                        ...
                        if (chr != null)
                        {
                            GlobalHandler.SendTeleportedByCristalResponse(chr.Client, Id, (short)(newPos.X - Offset), (short)(newPos.Y - Offset));
                        ...
                        if (chr.IsInGroup)
                        {
                            foreach (var member in chr.Group)
                            {
                                if (member.Character == null)
                                    continue;
                                xpBoost += member.Character.Asda2ExpAmountBoost - 1f;
                            }
                        }
                        else
                        {
                            xpBoost += chr.Asda2ExpAmountBoost - 1;
                        }
                        baseXp = (int)(baseXp * (1 + xpBoost));
                        if (baseXp < 0)
                            baseXp = 1;
        ...
        readonly Asda2LootItem[,] _lootPositions = new Asda2LootItem[500, 500];
        public readonly List<Asda2Loot> Loots = new List<Asda2Loot>();
        private readonly Dictionary<Locale, List<Character>> _charactersByLocale = new Dictionary<Locale, List<Character>>();
        ...
        public void SpawnLoot(Asda2Loot loot)
        {
            ...
            /*if (loot.Looters != null)
                foreach (var asda2LooterEntry in loot.Looters)
                {
                    if (asda2LooterEntry == null || asda2LooterEntry.Owner == null)
                        continue;
                    /*if (asda2LooterEntry.Owner.Level >= ((Unit)loot.Lootable).Level + CharacterFormulas.MaxLvlMobCharDiff)
                    {
                        foreach (var asda2LootItem in loot.Items)
                        {
        ...
        private void DropLoot(Asda2Loot loot)
        {
            ...
            var npc = ((Unit)loot.Lootable);
            loot.Position = npc.Position;
            loot.Map = npc.Map;
            var itemsCount = loot.Items.Length;
            Loots.Add(loot);
            loot.LootPositions = FindFreeLootSlots(itemsCount, loot.Asda2Position.XY);
            if (loot.LootPositions == null)
            {
                LogUtil.WarnException("LootPosition is null {0}, {1}", loot.Asda2Position.XY, Name);
                loot.Dispose();
                return;
            }
            for (int i = 0; i < loot.Items.Length; i++)
            {
                var pos = loot.LootPositions[i];
                loot.Items[i].Position = pos;
                _lootPositions[pos.X, pos.Y] = loot.Items[i];
            }
            AddObjectLater(loot);
        }
        ...
        {
            var r = new Vector2Short[itemsCount];
            short i = 0;
            var initXY = new Vector2Short((short)xy.X, (short)xy.Y);
            if (_lootPositions[initXY.X, initXY.Y] == null)
            {
                r[i] = initXY;
                i++;
                if (i >= itemsCount)
            ...
            while (true)
            {
                var positions = FindFreePositionsWithOffset(initXY, offset, (short)(r.Length - i));
                foreach (var t in positions)
                {
                    r[i++] = t;
                }
                if (i >= r.Length || offset > 10)
                    break;
                offset++;
            }
            return r;
        ...
        IEnumerable<Vector2Short> FindFreePositionsWithOffset(Vector2Short init, short offset, short count)
        {
            try
            {

                var r = new List<Vector2Short>(8);
                var i = 0;
                if (_lootPositions[init.X + offset, init.Y] == null)
                {
                    r.Add(new Vector2Short((short)(init.X + offset), init.Y));
                    i++;
                    if (i == count)
                        return r;
                }
                ...
                {
                    r.Add(new Vector2Short((short)(init.X + offset), (short)(init.Y + offset)));
                    i++;
                    if (i == count)
                        return r;
                }
                ...
                {
                    r.Add(new Vector2Short(init.X, (short)(init.Y + offset)));
                    i++;
                    if (i == count)
                        return r;
                }
            ...
            {
                var r = new List<Vector2Short>(count);
                for (var i = 0; i < count; i++)
                {
                    r.Add(init);
                }
