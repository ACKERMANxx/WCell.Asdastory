...
{
    [ActiveRecord(Table = "Asda2QuestNpc", Access = PropertyAccess.Property)]
    public class Asda2QuestNpc : WCellRecord<Asda2QuestNpc>
    {
        private static readonly NHIdGenerator _idGenerator =
            new NHIdGenerator(typeof(Asda2QuestNpc), "Id");
        ...
        public Asda2QuestNpc(long id, int npcId, int questNum, int questId, string questName)
        {
            Id = _id;
            NpcId = npcId;
            QuestNum = questNum;
        ...
        public static Asda2QuestNpc GetRecordByID(long Id)
        {
        ...
        public static int GetQuest(int npcId, int questNum)
        {
            var sql = string.Format("SELECT {0} FROM {1} WHERE {2} = {3} AND {4} = {5} LIMIT 1",
                ...
                DatabaseUtil.Dialect.QuoteForTableName(typeof(Asda2QuestNpc).Name),
                DatabaseUtil.Dialect.QuoteForColumnName("npcid"),
                npcId,
                DatabaseUtil.Dialect.QuoteForColumnName("questnum"),
                questNum);
            var query = new ScalarQuery<int>(typeof(Asda2QuestNpc), QueryLanguage.Sql, sql);
