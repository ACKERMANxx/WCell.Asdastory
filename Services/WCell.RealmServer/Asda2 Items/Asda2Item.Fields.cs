...
*/
        ...
        public byte EnchantChances
        {
            get { return (byte)(IsDeleted ? 0 : m_record.EnchantChances); }
            set
            {
                if (value == EnchantChances)
                    return;
                m_record.EnchantChances = value;
            }
        }

        public byte EnchantCount
        {
            get { return (byte)(IsDeleted ? 0 : m_record.EnchantCount); }
            set
            {
                if (value == EnchantCount)
                    return;
                m_record.EnchantCount = value;
            }
        }

        public byte EnchantLevel
        {
            get { return (byte)(IsDeleted ? 0 : m_record.EnchantLevel); }
            set
            {
                if (value == EnchantLevel)
                    return;
                m_record.EnchantLevel = value;
            }
        }

        public byte FailedEnchants
        {
            get { return (byte)(IsDeleted ? 0 : m_record.FailedEnchants); }
            set
            {
                if (value == FailedEnchants)
                    return;
                m_record.FailedEnchants = value;
            }
        }
