...
using WCell.RealmServer.AsdaStoryStats;
...
namespace WCell.RealmServer.Entities
{
    public partial class Asda2Item : ObjectBase, IOwned, IAsda2Weapon, INamed, ILockable, IQuestHolder, IAsda2MountableItem, IContextHandler
    {
        ...
        public void OnEquip()
        {
            ...
            if (Soul1Id != 0 && ItemId < 20732 && ItemId > 20751)
        ...
        private void ModifyStat(Asda2ItemBonusType type, int value)
        {
            ...
            switch (type)
            {
                ...
                case Asda2ItemBonusType.EmptySlot:
                    break;
                case Asda2ItemBonusType.MinMaxAtack:
                    OwningCharacter.ChangeModifier(StatModifierInt.Damage, (int)(value * CharacterFormulas.MaxToTotalMultiplier));
                    break;
                case Asda2ItemBonusType.MinMaxMAtack:
                    OwningCharacter.ChangeModifier(StatModifierInt.MagicDamage, (int)(value * CharacterFormulas.MaxToTotalMultiplier));
                    break;
                case Asda2ItemBonusType.MinMaxDeffence:
                    OwningCharacter.ChangeModifier(StatModifierInt.Asda2Defence, (int)(value * CharacterFormulas.MaxToTotalMultiplier));
                    break;
                ...
                case Asda2ItemBonusType.MinBlockRatePrc:
                    OwningCharacter.ChangeModifier(StatModifierInt.BlockValue, (int)(value * CharacterFormulas.MaxToTotalMultiplier) / 100);
                    break;
                case Asda2ItemBonusType.MaxBlockRatePrc:
                    OwningCharacter.ChangeModifier(StatModifierInt.BlockChance, (int)(value * CharacterFormulas.MaxToTotalMultiplier) / 100);
                    break;
                case Asda2ItemBonusType.MinMaxBlockRatePrc:
                    OwningCharacter.ChangeModifier(StatModifierInt.BlockChance, value * 2);
                    break;
        ...
        protected internal virtual void DoDestroy()
        {
            ...
            Asda2InventoryHandler.UpdateItemInventoryInfo(m_owner.Client, this);
            if (record != null)
            {
                record.OwnerId = 0;
                record.DeleteLater();
        ...
        public void GenerateNewOptions()
        {
            ...
            if (ItemOptionValueTemplate.ItemOptionValueTemplates.ContainsKey(this.Template.Stat1))
            {

                var types = ItemOptionValueTemplate.ItemOptionValueTemplates[this.Template.Stat1];
                if (types.Count >= 1)
                {
                    var x = ran.Next(types.Count);
                    Parametr1Type = (Asda2ItemBonusType)types[x].Type;
                    if (types[x].ValueOne == types[x].ValueTwo)
                        Parametr1Value = (short)types[x].ValueOne;
                    else
                    {
                        if (Parametr1Value == 0 && types[x].ValueTwo == 0)
                            Parametr1Value = (short)types[x].ValueOne;
                        if (Parametr1Value == 0 && types[x].ValueOne == 0)
                            Parametr1Value = (short)types[x].ValueTwo;
                        if (Parametr1Value == 0)
                            Parametr1Value = (short)ran.Next(types[x].ValueOne == 0 ? types[x].ValueTwo : types[x].ValueOne, types[x].ValueTwo == 0 ? types[x].ValueOne : types[x].ValueTwo);
                    }
                }
            }
            if (ItemOptionValueTemplate.ItemOptionValueTemplates.ContainsKey(this.Template.Stat2))
            {

                var types = ItemOptionValueTemplate.ItemOptionValueTemplates[this.Template.Stat2];
                if (types.Count >= 1)
                {
                    var x = ran.Next(types.Count);
                    Parametr2Type = (Asda2ItemBonusType)types[x].Type;
                    if (types[x].ValueOne == types[x].ValueTwo)
                        Parametr2Value = (short)types[x].ValueOne;
                    else
                    {
                        if (Parametr2Value == 0 && types[x].ValueTwo == 0)
                            Parametr2Value = (short)types[x].ValueOne;
                        if (Parametr2Value == 0 && types[x].ValueOne == 0)
                            Parametr2Value = (short)types[x].ValueTwo;
                        if (Parametr2Value == 0)
                            Parametr2Value = (short)ran.Next(types[x].ValueOne == 0 ? types[x].ValueTwo : types[x].ValueOne, types[x].ValueTwo == 0 ? types[x].ValueOne : types[x].ValueTwo);
                    }
                }
            }
            /*if (Record.IsCrafted)
        ...
        public void GenerateOptionsByCraft()
        {
            ...
            {
                var x = ran.Next(types.Count);
                Parametr3Type = (Asda2ItemBonusType)types[x].Type;
                if (types[x].ValueOne == types[x].ValueTwo)
                    Parametr3Value = (short)types[x].ValueOne;
                else
                {
                    if (Parametr3Value == 0 && types[x].ValueTwo == 0)
                        Parametr3Value = (short)types[x].ValueOne;
                    if (Parametr3Value == 0 && types[x].ValueOne == 0)
                        Parametr3Value = (short)types[x].ValueTwo;
                    if (Parametr3Value == 0)
                        Parametr3Value = (short)ran.Next(types[x].ValueOne == 0 ? types[x].ValueTwo : types[x].ValueOne, types[x].ValueTwo == 0 ? types[x].ValueOne : types[x].ValueTwo);
                }
            }
        ...
        {
            var types = ItemOptionValueTemplate.ItemOptionValueTemplates[this.Template.Stat1];
            types.AddRange(ItemOptionValueTemplate.ItemOptionValueTemplates[this.Template.Stat2]);
            if (types.Count >= 1)
            {
                var x = ran.Next(types.Count);
                Parametr4Type = (Asda2ItemBonusType)types[x].Type;
                if (types[x].ValueOne == types[x].ValueTwo)
                    Parametr4Value = (short)types[x].ValueOne;
                else
                {
                    if (Parametr4Value == 0 && types[x].ValueTwo == 0)
                        Parametr4Value = (short)types[x].ValueOne;
                    if (Parametr4Value == 0 && types[x].ValueOne == 0)
                        Parametr4Value = (short)types[x].ValueTwo;
                    if (Parametr4Value == 0)
                        Parametr4Value = (short)ran.Next(types[x].ValueOne == 0 ? types[x].ValueTwo : types[x].ValueOne, types[x].ValueTwo == 0 ? types[x].ValueOne : types[x].ValueTwo);
                }
            }
        ...
        {
            var types = ItemOptionValueTemplate.ItemOptionValueTemplates[this.Template.Stat2];
            types.AddRange(ItemOptionValueTemplate.ItemOptionValueTemplates[this.Template.Stat1]);

            if (types.Count >= 1)
            {
                var x = ran.Next(types.Count);
                Parametr5Type = (Asda2ItemBonusType)types[x].Type;
                if (types[x].ValueOne == types[x].ValueTwo)
                    Parametr5Value = (short)types[x].ValueOne;
                else
                {
                    if (Parametr5Value == 0 && types[x].ValueTwo == 0)
                        Parametr5Value = (short)types[x].ValueOne;
                    if (Parametr5Value == 0 && types[x].ValueOne == 0)
                        Parametr5Value = (short)types[x].ValueTwo;
                    if (Parametr5Value == 0)
                        Parametr5Value = (short)ran.Next(types[x].ValueOne == 0 ? types[x].ValueTwo : types[x].ValueOne, types[x].ValueTwo == 0 ? types[x].ValueOne : types[x].ValueTwo);
                }
            }
