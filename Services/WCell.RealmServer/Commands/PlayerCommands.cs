...
namespace WCell.RealmServer.Commands
{
    ...
    #endregion
    #region HideGM
    public class HideModeCommand : RealmServerCommand
    {
        protected HideModeCommand() { }
        ...
        protected override void Initialize()
        {
            Init("Hide", "HideGM");
            EnglishParamInfo = "[0|1]";
            EnglishDescription = "Toggles the HideMode";
        }

        public override void Process(CmdTrigger<RealmServerCmdArgs> trigger)
        {
            var target = (Character)trigger.Args.Target;
            var mode = (!trigger.Text.HasNext && target.IsGMVisible) || trigger.Text.NextBool();
            target.IsGMVisible = !mode;
            if (!target.IsGMVisible)
            {
                if (target.GodMode == false)
                target.GodMode = true;
            }
            trigger.Reply("GodMode " + (target.GodMode ? "ON" : "OFF"));
            trigger.Reply("HideMode " + (mode ? "ON" : "OFF"));
            ...
            /*if (mode == true)
            {
                using (var packet = new RealmPacketOut(RealmServerOpCode.StartMoveCommon))//4007
                {
                    ...
                    packet.WriteInt16(target.SessionId); //default value : 0
