...
namespace WCell.RealmServer.Commands
{
	...
	public class BroadcastCommand : RealmServerCommand
	{
		...
		protected override void Initialize()
		{
			base.Init(new string[] { "Broadcast", "Brc" });
			base.EnglishParamInfo = "<text>";
			base.EnglishDescription = "Broadcasts the given text throughout the world.";
		}
		...
		public override void Process(CmdTrigger<RealmServerCmdArgs> trigger)
		{
			...
			if (trigger.Text.Remainder.Length == 0)
			{
				trigger.Reply("Empty broadcast!");
				return;
			}
			string message = trigger.Text.Remainder;
			char[] chrArray = new char[] { ' ' };
			if ((int)message.Split(chrArray).Length == 0)
			{
				trigger.Reply("Empty broadcast!");
				return;
			}
			ChatMgr.SendGlobalMessageGM(trigger.Text.Remainder);
		}
	}
