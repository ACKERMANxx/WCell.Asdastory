...
namespace WCell.RealmServer.Asda2Fishing
{
    public static class Asda2FishingHandler
    {
        ...
        public static void StartFishingRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.SendErrorMsg(string.Format("Inter = {0}", interator));
            ...
            var FishLength = Util.Utility.Random(fish.MinLength, fish.MaxLength);
            SendSomeOneStartedFishingResponse(client.ActiveCharacter, (int)fish.ItemTemplate.Id, (short)FishLength);
        ...
        public static void SendFishingStartedResponse(IRealmClient client, Asda2StartFishStatus status, Int16 interator = 0, uint fishId = 0)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.FishingStarted))//6168
            {
                ...
                packet.WriteInt32(interator);//{interator}default value : 5 Len : 2
                client.Send(packet, addEnd: true);
            }
        }

        [PacketHandler(RealmServerOpCode.FishArrows)]
        public static void FishArrowsRequest(IRealmClient client, RealmPacketIn packet)
        {
            packet.Position -= 2;
            var arrows = packet.ReadByte();
            var fisharrows = client.ActiveCharacter.FishArrows;
            fisharrows = 0;
            if (client.ActiveCharacter.MapId == Constants.World.MapId.Alpia || client.ActiveCharacter.MapId == Constants.World.MapId.RainRiver)
            {
                if (arrows < 3)
                {
                    fisharrows += 1;
            ...
            if (client.ActiveCharacter.MapId == Constants.World.MapId.Silaris || client.ActiveCharacter.MapId == Constants.World.MapId.SunnyCoast)
            {
                if (arrows < 4)
                {
                    fisharrows += 1;
            ...
            SendFishArrowsResponse(client.ActiveCharacter, arrows);
        }

        public static void SendFishArrowsResponse(Character chr, byte arrows)
        {
            using (var packet = new RealmPacketOut(RealmServerOpCode.FishArrowsResponse))
            {
                packet.WriteByte(1);//{status}default value : 1 Len : 1
                packet.WriteInt16(1);//{fishId}default value : 31720 Len : 4
                packet.WriteInt16(1);//{fishId}default value : 31720 Len : 4
        ...
        public static void EndFishingRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            var success = CharacterFormulas.CalcFishingSuccess(client.ActiveCharacter.FishingLevel, spot.RequiredFishingLevel, client.ActiveCharacter.Asda2Luck, client.ActiveCharacter.FishArrows);
            if (!success)
            {
            ...
            client.ActiveCharacter.FishArrows = 0;
        ...
        public static void RegisterFishingBookRequest(IRealmClient client, RealmPacketIn packet)
        {
            ...
            client.ActiveCharacter.SendErrorMsg("" + cell);
            if (itm == null)
            {
