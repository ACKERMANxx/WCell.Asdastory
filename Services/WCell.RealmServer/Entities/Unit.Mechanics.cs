...
namespace WCell.RealmServer.Entities
{
    ...
    public partial class Unit
    {
        ...
        public static float DefaultRunSpeed = 0.37f;//7f;
        ...
        public void TeleportTo(Map map, ref Vector3 pos, float? orientation)
        {
            ...
            if (chr != null)
            {
                ...
                Log.Create(Log.Types.ChangePosition, LogSourceType.Character, chr.EntryId)
                    .AddAttribute("source", 0, "teleport")
                   .AddAttribute("map", (double)map.Id, map.Id.ToString())
                   .AddAttribute("x",pos.X)
                   .AddAttribute("y",pos.Y)
                   .Write();}
            ...
            if (ownerMap == map)
            {
                if (Map.MoveObject(this, ref pos))
                {
                    ...
                    if (chr != null)
                    {
                        ...
                        //Asda2MovmentHandler.SendEndMoveCommonResponse(chr);
        ...
        public float IsCollisionWith(Unit unit)
        {
            ...
            {
                unit.BoundingRadius = 0.3f;
                unit.BoundingCollisionRadius = 0.61f;
            }
            else
            {
                if (unit.BoundingRadius < 1f)
                {
                    unit.BoundingRadius = 1.5f;
                    unit.BoundingCollisionRadius = 2.501f;
                }
            }*/
            var distX = Position.X - unit.Position.X;
            var distY = Position.Y - unit.Position.Y;
            var dist = (float)Math.Sqrt((distX * distX) + (distY * distY));
            return BoundingRadius + unit.BoundingRadius - dist;
        }
    }
}