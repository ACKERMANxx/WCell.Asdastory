...
namespace WCell.RealmServer.Entities
{
    ...
    public partial class Character
    {
                ...
                Observing.IterateEnvironment(BroadcastRange, (obj) =>
                {
                    ...
                    if (!KnownObjects.Contains(obj))
                    {
                        ...
                        if (visibleChr != null && visibleChr != this)
                        {
                            ...
                            {
                                Map.CallDelayed(400, () => FunctionalItemsHandler.SendShopItemUsedResponse(Client, visibleChr, int.MaxValue));
                            }
                            if (visibleChr.IsOnMount)
                                Map.CallDelayed(500, () => Asda2MountHandler.SendCharacterOnMountStatusChangedToPneClientResponse(Client,
                                    visibleChr));
                        }
                        else
                        {
        ...
        {
            if (LastPingTime == null || LastPingTime == DateTime.MinValue)
                return;
            if (DateTime.Now - LastPingTime > new TimeSpan(0, 0, 20))
            {
                Logout(true);
                if (this.Client != null)
        ...
        public override void Update(int dt)
        {
            ...
            //CheckPing();
            if (DateTime.Now.Subtract(new TimeSpan(0,0,0,30)) > LastCharge)
            {
                bool change = false;
                if (GreenCharges > 0)
                {
                ...
                if (change)
                {
                    ...
                    Asda2SpellHandler.CheckSoulGuard(this, -1);
                }
            }
