...
namespace WCell.RealmServer.Entities
{
	...
	public abstract partial class Unit : WorldObject, ILivingEntity, ISummoner
	{
		...
		protected void Die(bool force)
		{
            ...
            if (chr != null)
                Asda2CharacterHandler.SendHealthUpdate(chr);
            ...
            if(chr != null)
               if(Map!=null)
                   Map.CallDelayed((int) LastDamageDelay,()=>Asda2CharacterHandler.SendSelfDeathResponse(chr));
		...
		/// </summary>
		public float PowerRegenPerTickActual
		{
			get;
			internal set;
		}
		...
		protected void Regenerate(int dt)
		{
	        ...
	        if (IsSitting)
	        {
	            healthRegen = MaxHealth*0.009f;
	        }
	        else
	        {
	            healthRegen = m_isInCombat ? HealthRegenPerTickCombat : HealthRegenPerTickNoCombat;
	        }
	        healthRegen = healthRegen * dt / RegenerationFormulas.RegenTickDelayMillis;
		    _tempHealthRegen += healthRegen;
		    _timeFromLastHealthUpdate += dt;
            if (_timeFromLastHealthUpdate > DefaultHealthAndPowerUpdateTime)
            {
                _timeFromLastHealthUpdate -= DefaultHealthAndPowerUpdateTime;
                if (_tempHealthRegen > 1)
                {
                    var intHealth = (int)_tempHealthRegen;
                    _tempHealthRegen -= intHealth;
                    if (Health < MaxHealth)
                    {
                        Health = oldHealth + intHealth;
                        var chr = this as Character;
                        if (chr != null)
                        {
                            Asda2CharacterHandler.SendHealthUpdate(chr);
		...
		public void Heal(int value, Unit healer = null, SpellEffect effect = null)
        {
			...
			if (effect != null)
			{
			    ...
			    if (character != null)
				{
					value += (int)((oldVal * character.HealingTakenModPct) / 100);
				}
			...
			if (value > 0)
			{
                ...
                if (character != null)
                value += (int)((value * character.HealingTakenModPct) / 100);
				...
				value += overheal;
                if(character!=null)
                Asda2CharacterHandler.SendHealthUpdate(character,true);
		...
		protected virtual void OnHeal(HealAction action)
		{
			...
			this.IterateEnvironment(15.0f, obj =>
			{
