...
namespace WCell.RealmServer.Entities
{
	...
	public partial class Character : Unit, IUser, IContainer, ITicketHandler, IInstanceHolderSet, ICharacterSet
	{
		...
		/// </summary>
		[NotVariable]
		public static int DefaultLogoutDelayMillis = 15000;
		...
		protected override bool OnBeforeDeath()
		{
			...
			if (m_Map != null && !m_Map.MapTemplate.NotifyPlayerBeforeDeath(this))
			{
				return false;
			}
		...
		protected override void OnDeath()
		{
            ...
            if (!RemoveDeathPenalties && !IsAsda2BattlegroundInProgress)
            {
		...
		protected override void OnKilled(IDamageAction action)
		{
            ...
            if(action.Attacker!=null)
            {
                ...
                if (killer != null)
                {
                    ...
                    {
						Console.WriteLine(killer.Name + " " + Name);
						...
						Asda2BattlegroundHandler.SendSomeOneKilledSomeOneResponse(killer.CurrentBattleGround, points, killer, this);
                    }
                }
            }
			/*bool pvp;
		...
		protected override void OnHeal(HealAction action)
		{
			...
			if (healer is Character)
			{
				...
				if (chr.IsInBattleground)
				{
					chr.Battlegrounds.Stats.TotalHealing += (action.Value) * 4;//healinghere
				}
			}
		}
		...
		public void GainCombatXp(int experience, INamed killed, bool gainRest)
		{
			...
			if (npc.EntryId == 328 || npc.EntryId == 327)
				xp = 1;
			Experience += xp;
		...
		protected override void OnLevelChanged()
		{
		    ...
		    Log.Create("gain_stats", LogSourceType.Character, EntryId)
               ...
               .Write();
			//SendSystemMessage(String.Format("You have {0} free stat points. Enter \"#HowToAddStats\" to see help.",FreeStatPoints));
			if ((Asda2BaseSpirit == 1 || Asda2BaseStamina == 1 || Asda2BaseStrength == 1 || Asda2BaseLuck == 1 || Asda2BaseAgility == 1 || Asda2BaseIntellect == 1) &&
				Level >= 2)
			{
			...
			if (IsSoulmated && SoulmateCharacter != null)
			{
				if (Level >= 15)
					SoulmateCharacter.AsdaStoryGetTitle((Asda2TitleId)43);
			}
		...
		public void imdogshit(int lvl)
		{
			Asda2BaseSpirit = lvl;
			Asda2BaseLuck = (2 * lvl);
			Asda2BaseIntellect = lvl;
			Asda2BaseAgility = lvl;
			Asda2BaseStamina = (10 * lvl);
			Asda2BaseStrength = lvl;
		}
        ...
        {
            //SendMessage("~المزاد~", msg, Color.DeepPink);
        }

        public void SendCraftingMsg(string msg)
        {
            //SendMessage("~الصناعة~", msg, Color.BurlyWood);
        }
        ...
        {
           // SendMessage("~نظام المهمات~", msg, Color.Gold);
        }

        public void SendErrorMsg(string msg)
        {
            //SendMessage("~خطا~", msg, Color.Red);
            log.Warn(string.Format("error msg to character : {0}.\r\n{1}", Name, msg));
        }

        public void SendWarnMsg(string msg)
        {
            //SendMessage("~انتباه~", msg, Color.Red);
        }

        public void SendWarMsg(string msg)
        {
            //SendMessage("~الحرب~", msg, Color.OrangeRed);
        }

        public void SendMailMsg(string msg)
        {
            //SendMessage("~البريد~", msg, Color.Gainsboro);
        }
        ...
        public QuestRecord[] m_questRecordz;
        public QuestRecord[] ActiveQuests
        {
            get { return m_questRecordz; }
        }
        #endregion
		...
		public bool CanPvP(Character chr)
        {
            if (!chr.IsAlive || chr.Map == null)
                return false;
            if (chr.Map.MapTemplate.IsAsda2FightingMap)
            {
		...
		{
			healValue += (int)((healValue * HealingDoneModPct) / 100f)*2;
		...
		public List<Character> getCharactersNearby(bool includeSelf = false)
		{
			List<Character> chrs = new List<Character>();
			var gets = this.Map.GetObjectsInRadius<Character>(this.Position, WorldObject.BroadcastRange, this.Phase);
			if (includeSelf)
				chrs.Add(this);
			foreach (Character chr in gets)
			{
				chrs.Add(chr);
			}
			return chrs;
		...
		//here3
		public virtual void SendPacketToNear(RealmPacketOut packet, bool includeSelf = false)
		{
			var chrs = getCharactersNearby(includeSelf);
