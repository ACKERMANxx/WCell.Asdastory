...
namespace WCell.RealmServer.Entities
{
    ...
    public abstract partial class WorldObject : ObjectBase, IFactionMember, IWorldLocation, INamedEntity, IContextHandler
    {
        ...
        public UInt16 UniqIdOnMap { get; set; }
        /// <summary>
        /// All phases
        /// </summary>
        ...
        /// </summary>
        public static float BroadcastRange = 50.0f;
        public static float BroadcastRangeNpc = 35.0f;
        ...
        public float Asda2X
        {
            get { return Map == null ? 0 : Position.X - Map.Offset; }
            set { Position = new Vector3(value + Map.Offset, Position.Y); }
        }

        public float Asda2Y
        {
            get { return Map == null ? 0 : Position.Y - Map.Offset; }
        ...
        public float GetSpellMaxRange(Spell spell, WorldObject target, float range)
        {
            ...
            if (this is Unit)
            {
                range += ((Unit)this).CombatReach + ((Unit)this).IntMods[(int)StatModifierInt.CastingDistance];
        ...
        public void Say(string message)
        {
            ...
            //using (var p = ChatMgr.CreateNormalChatMessagePacket(Name, message, false))
            //{
            //    SendPacketToArea(p, false, false);
            //}
        }
        ...
        public virtual void SendPacketToArea(RealmPacketOut packet, bool includeSelf = true, bool addEnd = true, Locale locale = Core.Network.Locale.Any, float? radius = null)
        {
            //Console.WriteLine("Before Send: " + packet.ToHexDump());
            if (Map == null || Thread.CurrentThread.ManagedThreadId != m_Map.CurrentThreadId)
                return;
            var range = radius ?? BroadcastRange;
            if (IsAreaActive)
            {
                if (locale == Core.Network.Locale.Any)
                    for (var i = m_Map.Characters.Count - 1; i >= 0; i--)
                    {
                        var character = m_Map.Characters[i];
                        if(!includeSelf && Equals(character))
                            continue;
                        if (range > GetDistance(character))
                            character.Send(packet, addEnd: addEnd);
                    }
                else
                {
                    for (var i = m_Map.CharactersByLocale[locale].Count - 1; i >= 0; i--)
                    {
                        var character = m_Map.CharactersByLocale[locale][i];
                        if (!includeSelf && Equals(character))
                            continue;
                        if (range > GetDistance(character))
                            character.Send(packet, addEnd: addEnd);
            ...
            //Console.WriteLine("After Send: " + packet.ToHexDump());
        ...
        {
            using (var p = ChatMgr.CreateGlobalChatMessage(Name, msg, c, Core.Network.Locale.En))
            {
                SendPacketToArea(p, true, false, Core.Network.Locale.Any);
            }
        }
    }
}